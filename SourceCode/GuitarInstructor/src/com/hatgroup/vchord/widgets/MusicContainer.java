...
public class MusicContainer extends ViewGroup {
	...
	public MusicContainer(Context context, AttributeSet attrs) {
		super(context, attrs);
		TypedArray styledAttributes = context.obtainStyledAttributes(attrs,
				R.styleable.MusicContainer);
		horizontalSpacing = styledAttributes.getDimensionPixelSize(
				R.styleable.MusicContainer_android_horizontalSpacing,
				DEFAULT_HORIZONTAL_SPACING);
		verticalSpacing = styledAttributes.getDimensionPixelSize(
				R.styleable.MusicContainer_android_verticalSpacing,
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		for (View child : getLayoutChildren()) {
			...
			//	rows.add(currentRow);
			//}
			boolean exceedMax = currentRow.wouldExceedMax(childWidth);
			if (exceedMax) {
				currentRow = new RowMeasurement(maxInternalWidth, widthMode);
				rows.add(currentRow);
			}				
	...
	protected void onLayout(boolean changed, int leftPosition, int topPosition,
		...
		for (View child : getLayoutChildren()) {
			...
			//if (x + childWidth > widthOffset || hasNewLine(child)) {
	...
	private final class RowMeasurement {
		...
		public boolean wouldExceedMax(int childWidth) {
			if(widthMode == MeasureSpec.UNSPECIFIED){
				...
				int newWidth = getNewWidth(childWidth);
